...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    name: "grep",
    label: "ripgrep as grep override",
    // Document the truncation limits in the tool description so the LLM knows
    description: `Ripgrep but represented as grep in tool call. Output is truncated to ${DEFAULT_MAX_LINES} lines or ${formatSize(DEFAULT_MAX_BYTES)} (whichever is hit first). If truncated, full output is saved to a temp file.`,
    parameters: RgParams,
